...
\textit{Proof:} The algorithm (\texttt{\nameref{alg:mPNE}} (line 1 to 5)) checks the lower bound for every $r_1$ neighbor of $sp$ in the category set $C_{M_1}$. All partial routes, which satisfy the requirement to have a shorter lower bound than the estimated upper bound are given a chance to be expanded and placed on the heap. 
%Let us assume that there is a route $R = ()$
The correctness of the algorithm is implied by the correctness of PNE itself.
%??? running until the heap is empty
